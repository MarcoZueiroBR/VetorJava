...
public class Vetor {
    ...
    // Método para remover uma posição do Vetor, e trazendo os elementos à direta para a esquerda.
    ...
    // Método para remover por busca de elemento.
    public boolean removerPorBusca(String elemento) {
        if (buscarElemento(elemento) == true) {
            for (int i = 0; i < this.lista.length; i++) {
    ...
    // Método para encontrar a ultima ocorrência do elemento no Vetor.
    ...
    // Método para remover todos as ocorrências do elemento.
    public void removerTodos(String elemento) {
        while(removerPorBusca(elemento));
    }

    // Método par adicionar caso o elemento não exista no vetor.
    public boolean adicionarSeNaoExiste(String elemento){
        if (this.buscarElemento(elemento) == false){
    ...
    private void aumentaCapacidade(){
        if(this.tamanho == this.lista.length){
            String[] elementosNovos = new String[this.lista.length * 2];
            for (int i = 0; i < this.tamanho; i++) {
                elementosNovos[i] = this.lista[i];
            }
            this.lista = elementosNovos;
        }
    }

    // Método auxiliar
    private void adiciona(int posicao, String elemento) {
    ...
    // Método para inserir um elemento depois de outro ter ocorrido no vetor.
    public boolean inserirDepois(String referencia, String novoElemento){
        int posicao = this.ultimoIndice(referencia);
        if (posicao == -1){
            return false;
        }
        this.aumentaCapacidade();
